...
CRNN_CHECKPOINT = os.path.join(OUTPUT_ROOT, "checkpoints", "hwr", "WordLevelResNetCRNN.pth")
...
CRNN_CUSTOM_SAMPLES = os.path.join(BASE_DIR, "archive", "iam_words")
...
def calculate_stats(loader):
    ...
    for batch in tqdm(loader, desc="Obliczanie statystyk", leave=False, disable=True):
...
class PHSFDataset(Dataset):
    ...
    def __init__(self, root_dir, encoder, transform=None):
        ...
        # Mapujemy nazwy folderów (np. 'ą', 'Ć') na indeksy Twojego encodera
        for char_name in os.listdir(root_dir):
            char_path = os.path.join(root_dir, char_name)
            if not os.path.isdir(char_path): continue

            # Pobieramy indeks z rozszerzonego alfabetu (92 klasy)
            label_idx = self.encoder.char_to_idx.get(char_name)
            if label_idx is None: continue

            # Przeszukujemy pliki wewnątrz
            for img_file in glob.glob(os.path.join(char_path, "**", "*.png"), recursive=True):
    ...
    def __getitem__(self, idx):
        ...
        # PHSF wymaga inwersji
        if np.mean(img) > 127:
            img = cv.bitwise_not(img)

        if self.transform:
            # Resize 64x64, normalizacja
            img = self.transform(Image.fromarray(img))

        # Format 7-elementowy dla zgodności z CapsNet
        return (
            img,
            torch.tensor(label, dtype=torch.long),
            1,  # Strefa
            torch.zeros(7),  # Granice
            torch.zeros(1024),  # Kontekst
            torch.tensor(1.0),  # Pewność
            torch.zeros(74)  # Predykcje CRNN
        )
...
class CharLabelEncoder:
    ...
    def __init__(self):
        ...
        raw_chars = sorted([c for c in combined_list if c != ''])
...
class LossPlotter:
    """ Klasa odpowiedzialna za wizualizację i monitorowanie postępów treningu na żywo. Przechowuje historię funkcji
        straty, dokładności oraz błędu CER, a następnie generuje i aktualizuje wykresy w folderze diagnostycznym. """
    def __init__(self):
        self.history = {'loss': [], 'val_acc': [], 'val_cer': []}
        os.makedirs(DEBUG_DIR, exist_ok=True)

    def update(self, train_loss, val_acc, val_cer):
        self.history['loss'].append(train_loss)
        self.history['val_acc'].append(val_acc)
        self.history['val_cer'].append(val_cer)

        # Usuwanie starych plików przed zapisem nowej wersji
        for old_file in glob.glob(os.path.join(DEBUG_DIR, "training_metrics*.png")):
            try:
                os.remove(old_file)
            except OSError:
                pass
        ...
        ax1.plot(self.history['loss'], label='Train Loss', color='royalblue')
        ax1.set_ylabel('Loss')
        ax1_2 = ax1.twinx()
        ax1_2.plot(self.history['val_acc'], label='Val Acc', color='forestgreen')
        ax1_2.set_ylabel('Accuracy')
        plt.title('Progresja Treningu')

        # Wykres CER
        plt.subplot(1, 2, 2)
        plt.plot(self.history['val_cer'], label='Val CER', color='crimson')
        plt.title('Błąd Znaków (CER)')

        plt.tight_layout()
        plt.savefig(os.path.join(DEBUG_DIR, "training_metrics.png"))
        plt.close()
...
def visual_test(model, loader, device, encoder, num_images=15):
    """ Wizualizacja predykcji CapsNet z uwzględnieniem Deep Fusion.
    ...
    try:
        ...
        inputs, targets, zones, boundaries, context_vecs, confs, *rest = [
            b.to(device) if torch.is_tensor(b) else b for b in batch
        ]
        # Wyciągnięcie prawdopodobieństw CRNN z reszty batcha
    ...
    with torch.no_grad():
        # Wywołanie modelu z kontekstem dla lepszej skuteczności
        classes_norms, _, z_logits, _ = model(
            inputs,
            word_context=context_vecs,
            confidence=confs,
            crnn_probs=crnn_p,
...
class HardCharsDataset(Dataset):
    """ Dataset integrujący wycinki znaków (64x64) z metadanymi CRNN (.npy) oraz kontekstem językowym. """
    def __init__(self, root_dir, encoder, case_type="hard", transform=None):
        ...
        # Mapowanie folderów na indeksy
        idx_to_char = self.encoder.idx_to_char
        safe_name_to_idx = {get_safe_char_name(c): i for i, c in idx_to_char.items()}
        if 'background' not in safe_name_to_idx: safe_name_to_idx['background'] = 0

        for char_folder in os.listdir(root_dir):
            char_path = os.path.join(root_dir, char_folder)
            if not os.path.isdir(char_path): continue

            label_idx = safe_name_to_idx.get(char_folder)
            if label_idx is None: continue

            target_path = os.path.join(char_path, case_type)
    ...
    def __getitem__(self, idx):
        ...
        # Logika przełącznika języka na podstawie ścieżki
        lang_id = 0 if ("PHSF" in img_path or "pl_line" in img_path) else 1

        img = cv.imread(img_path, cv.IMREAD_GRAYSCALE)
        if img is None:
            print(f"\n Nie znaleziono obrazu pod ścieżką: {img_path}")
            return None

        img = img.astype(np.float32) / 255.0
        img = torch.from_numpy(img).unsqueeze(0)

        data = np.load(s['data'], allow_pickle=True).item()
        context_vec = torch.tensor(data=data.get('context_vector', np.zeros(1024)), dtype=torch.float32)
        crnn_probs = torch.tensor(data=data.get('crnn_probs', np.zeros(74)), dtype=torch.float32)

        return (
            img,
            torch.tensor(s['label'], dtype=torch.long),
            torch.tensor(1, dtype=torch.long), # Strefa
            torch.zeros(7, dtype=torch.float32), # Granice
            context_vec,
            torch.tensor(1.0, dtype=torch.float32), # Pewność
            crnn_probs,
            torch.tensor(lang_id, dtype=torch.long) # Identyfikator języka
        )


class BalancedLazyDataset(Dataset):
    def __init__(self, emnist_paths, pure_paths, transform=None):
        # Ścieżki do plików
        self.emnist_paths = emnist_paths
        self.pure_paths = pure_paths
        self.transform = transform

        # Opieramy wielkość wirtualnej epoki na większym zbiorze
        self.length = max(len(self.emnist_paths), len(self.pure_paths))

    def __len__(self):
        # Mnożymy x2, ponieważ każdy krok w `__getitem__` zwraca jedną próbkę,
        # a chcemy wyciągnąć łącznie tyle samo eMNIST co PURE
        return self.length * 2

    def __getitem__(self, idx):
        # Indeksy parzyste to eMNIST, nieparzyste to PURE.
        if idx % 2 == 0:
            path = random.choice(self.emnist_paths)
        else:
            path = random.choice(self.pure_paths)

        # Wczytujemy obraz z dysku
        image = Image.open(path).convert('L')

        # Tutaj wywołaj swoją funkcję do odczytania etykiety na podstawie nazwy pliku/ścieżki
...
def get_balanced_dataloader(emnist_paths, pure_paths, batch_size, transform=None):
    dataset = BalancedLazyDataset(emnist_paths, pure_paths, transform)

    return DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=True,
        # num_workers=0 jest KRYTYCZNE na Windowsie, by uniknąć cichego zawieszania na starcie
        num_workers=0,
        # pin_memory=True przyspiesza transfer danych na kartę graficzną, rekompensując odczyt z dysku
        pin_memory=True
    )
...
class GatedContextFusion(nn.Module):
    """ Gated Residual Multimodal Fusion wzbogacony o bezpośrednią opinię CRNN
        oraz geometryczny przełącznik języka (Bilingual Context). """
    def __init__(self, caps_dim=32, context_dim=1024, crnn_logits_dim=74):
        super().__init__()
        # Projektujemy kontekst na wymiar kapsułki
        self.project = nn.Linear(context_dim, caps_dim)

        # rnn_logits_dim musi być stałe, bo CRNN był trenowany na takim alfabecie
        self.prob_project = nn.Linear(crnn_logits_dim, caps_dim)

        # Pozwala CapsNetowi nadać priorytet polskim znakom diakrytycznym w fazie fuzji.
        self.lang_emb = nn.Embedding(2, caps_dim)

        # Bramka decyzyjna: Wizja (CapsNet) + Semantyka (Context) + Opinia (CRNN) + Język (ID)
        self.gate = nn.Sequential(
            nn.Linear(caps_dim * 4, caps_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(0.1),  # Regularyzacja fuzji
            nn.Linear(caps_dim, caps_dim),
            nn.Sigmoid()
        )

    def forward(self, caps_output, word_context, confidence, crnn_probs, lang_id):
        batch_size, num_caps, caps_dim = caps_output.size()

        # Projektujemy opinię CRNN, żeby CapsNet podejmował decyzję posiadając pełny kontekst
        crnn_prior = self.prob_project(crnn_probs).unsqueeze(1)
        crnn_prior_expanded = crnn_prior.expand(-1, num_caps, -1)

        # Projektujemy kontekst semantyczny
        ctx = self.project(word_context).unsqueeze(1)
        ctx_expanded = ctx.expand(-1, num_caps, -1)

        # Projektujemy identyfikator języka na wektor przestrzeni CapsNet
        lang_prior = self.lang_emb(lang_id).unsqueeze(1)
        lang_prior_expanded = lang_prior.expand(-1, num_caps, -1)

        # Łączymy cechy wizualne kapsułek ze wszystkimi źródłami wiedzy zewnętrznej
        combined = torch.cat([caps_output, ctx_expanded, crnn_prior_expanded, lang_prior_expanded], dim=-1)

        # Gating mechanism: brama decyduje, które źródło informacji jest najważniejsze dla danego znaku
        g = self.gate(combined)

        # Przygotowanie mnożnika pewności z CRNN
        conf_multiplier = confidence.view(batch_size, 1, 1)

        # Sumaryczna wiedza zewnętrzna wpływająca na orientację kapsułek
        external_knowledge = ctx_expanded + crnn_prior_expanded + lang_prior_expanded

        # Fuzja: Kapsułka zachowuje cechy geometryczne, ale jej aktywacja jest modulowana przez język
        fused = caps_output + (g * conf_multiplier * external_knowledge)

        return self.squash(fused)
...
class AttentionRouting(nn.Module):
    ...
    def forward(self, u_hat, num_iterations=None):
        # u_hat shape: [Batch, in_caps, out_caps, dim_caps]

        # Wyliczamy siłę dopasowania (Agreement) za pomocą atencji wewnątrz-kapsułkowej
...
class BoundarySpatialAttention(nn.Module):
    """ Mapuje granice czasowe z CRNN na przestrzenną maskę uwagi po osi X. """
    def __init__(self, feat_width=21):
        super().__init__()
    ...
    def forward(self, x, boundaries):
        ...
        # Zakładamy, że boundaries są znormalizowane 0-1
        start_rel = boundaries[:, 1].view(batch_size, 1, 1, 1)
        end_rel = boundaries[:, 2].view(batch_size, 1, 1, 1)

        # Tworzymy siatkę współrzędnych X dla całej mapy cech
        grid_x = torch.linspace(0, 1, w, device=x.device).view(1, 1, 1, w)

        # Tworzymy maskę, żeby uniknąć szumu
        mask = (grid_x >= start_rel) & (grid_x <= end_rel)
        mask = mask.float()

        # Wygładzanie krawędzi (antyaliasing uwagi)
        mask = func.avg_pool2d(mask, kernel_size=(1, 3), stride=1, padding=(0, 1))
...
class AffineSTN(nn.Module):
    ...
    def __init__(self, in_channels=1, input_size=32):
        ...
        self.loc_net = nn.Sequential(
            ...
            nn.MaxPool2d(2, stride=2),
            nn.Flatten(),
            nn.Linear(32 * (input_size // 4) * (input_size // 4), 64),
            nn.ReLU(True)
        )

        # Regresorparametrów afinicznych
...
class CapsNet(nn.Module):
    """ Implementacja Capsule Network dedykowana do rozpoznawania trudnych/niejednoznacznych znaków.
        1. GVF Restoration: Wstępna, fizyczna naprawa przerwanych pociągnięć znaków (powolna, ale potrzebna, jeśli
           CapsNet ma być specjalistą w ciężkich przypadkach).
        2. Feature Extractor: Zmodyfikowany ResNet z blokami Squeeze-and-Excitation.
           Służy do ekstrakcji głębokich, niezmienniczych cech wizualnych z obrazu wejściowego.
        3. Primary Capsules: Przekształca skalarne mapy cech z CNN na wektorowe kapsułki.
        4. Attention Routing: Zastępuje klasyczny 'Dynamic Routing' mechanizmem atencji.
           Pozwala to na znacznie szybsze trenowanie i lepszą selekcję informacji między warstwami kapsułek.
    ...
    def  __init__(self, num_classes=89, context_dim=1024):
        ...
        """ Zastosowanie afinicznego modułu przestrzennego przed wejściem do sieci kapsułkowej ma na celu
            przeprowadzenie aktywnej normalizacji geometrycznej. Sieć lokalizująca estymuje 6 parametrów,
            które tworzą macierz transformacji afinicznej. Koryguje to translację, skalę oraz pochylenie kursywy. 
            Proces ten drastycznie redukuje wariancję wewnątrzklasową, zwalniając algorytm 
            routingu w sieci CapsNet z konieczności modelowania deformacji kątowych. """
        self.stn = AffineSTN(in_channels=1, input_size=32)

        # Moduł rekonstrukcji topologii GVF + Balloon (Broken character restoration using Gradient Vector Flow and Balloon Force Algorithm)
        ...
        self.boundary_gate = BoundarySpatialAttention()
        ...
        # Fuzja
        self.fusion = GatedContextFusion(caps_dim=32, context_dim=context_dim, crnn_logits_dim=num_classes)

        # Głowice pomocnicze (uczenie morfologii litery)
        self.dropout = nn.Dropout(p=0.2)  # Do próbkowania cech wizualnych
        self.zone_head = nn.Sequential(
            nn.Linear(32, 64),
            nn.ReLU(True),
            nn.Dropout(p=0.3),  # Dropout dla MC w głowicy stref
            nn.Linear(64, 3)
        )
        self.geom_head = nn.Sequential(nn.Linear(32, 64), nn.ReLU(True), nn.Linear(64, 7))

        # Rekonstrukcja obrazu znaku
    ...
    def forward(self, x, y=None, word_context=None, confidence=None, crnn_probs=None, boundaries=None,
        ...
        """ Pełny przepływ w przód modelu CapsNet z fuzją kontekstową i analizą niepewności. """
        batch_size = x.size(0)
        ...
        # Ekstrakcja cech z wykorzystaniem Residual, SE i Deformable Blocks
        features = self.backbone(x_aligned)

        # Bramkowanie przestrzenne oparte na granicach znaków z CRNN
        ...
        classes_norms = (v_j ** 2).sum(dim=-1) ** 0.5

        # Softmax dla obliczenia entropii
        probs = torch.softmax(classes_norms, dim=-1)
        entropy = -torch.sum(probs * torch.log(probs + 1e-10), dim=-1)

        # Selekcja zwycięskiej kapsuły
        if y is None:
        ...
        # Głowice pomocnicze i dekoder rekonstrukcyjny
        reconstruction = self.decoder(masked_v)

        # Predykcja strefy (upper, core, lower) i statystyk geometrycznych
        z_logits = self.zone_head(masked_v)
        g_stats = self.geom_head(masked_v)

        return {
            "norms": classes_norms,  # Prawdopodobieństwa klas
            "probs": probs,  # Rozkład softmax (do entropii)
            "entropy": entropy,  # Niepewność aleatoryczna
            "reconstruction": reconstruction,  # Odtworzony obraz znaku
            "z_logits": z_logits,  # Klasyfikacja strefy litery
            "g_stats": g_stats,  # Statystyki geometryczne znaku
            "capsules": v_j  # Pełne wektory kapsuł
        }

    def freeze_backbone(self):
        # Mrozimy tylko pierwsze warstwy, które wyłapują proste krawędzie
    ...
    def predict_char(self, crop_tensor, encoder, word_context=None, confidence=None, boundaries=None, crnn_probs=None, mc_samples=10):
        """ Zwraca szczegółową predykcję wraz z wektorem kapsułki dla modelu językowego.
            Wykorzystuje technikę Monte Carlo Dropout do estymacji niepewności modelu. Wielokrotne przepuszczenie
            obrazu przez sieć pozwala obliczyć zarówno niepewność epistemiczną (niewiedzę modelu),
            jak i aleatoryczną (niejednoznaczność samego znaku). Te są kluczowe dla uwagi w Transformerze. """
        self.eval()
        all_probs = []
        all_capsules = []
        ...
        with torch.no_grad():
            for _ in range(mc_samples):
                # Naprawa (GVF) -> Rektyfikacja (STN) -> Ekstrakcja (Backbone)
                ...
                # Klasyfikacja oparta na długości wektorów kapsułek
        ...
        # Agregacja wyników z próbkowania stochastycznego
        ...
        for target_class in self.classes:
            ...
            for f in os.listdir(class_path):
                if f.endswith('.png'):
                    ...
                    # Zakładamy, że .npy ma tę samą nazwę co .png
...
class UncertaintyAwareMarginLoss(nn.Module):
    """ Margin Loss ważona niepewnością CRNN.
        Dla pipeline CRNN → CapsNet:
        - Wysoka pewność CRNN → Normalna waga loss (CapsNet uczy się od dobrej predykcji)
        - Niska pewność CRNN → Podwyższona waga loss (CapsNet musi nauczyć się trudnych przypadków) """
    def __init__(self, num_classes, device, gamma=2.0, smoothing=0.1):
        super().__init__()
        self.num_classes = num_classes
        self.device = device
        self.gamma = gamma
    ...
    def forward(self, probs, labels, crnn_confidence=None):
        # One-hot z wygładzaniem etykiet (label smoothing)
        labels_one_hot = torch.eye(self.num_classes, device=self.device)[labels]
        labels_one_hot = labels_one_hot * (1.0 - self.smoothing) + (self.smoothing / self.num_classes)

        # Składowe funkcji straty Margin Loss (standardowe)
        m_pos = torch.pow(torch.relu(0.9 - probs), 2)
        m_neg = torch.pow(torch.relu(probs - 0.1), 2)

        # Ważenie fokalne (skupienie na trudnych próbkach wewnątrz CapsNet)
        pt = (probs * labels_one_hot + (1 - probs) * (1 - labels_one_hot))
        focal_weight = torch.pow(1 - pt, self.gamma)

        # Bazowa strata Margin Loss
        margin_loss = (labels_one_hot * m_pos + 0.5 * (1.0 - labels_one_hot) * m_neg)
        margin_loss = (focal_weight * margin_loss).sum(dim=1)

        # Odwrotność pewności modelu CRNN
        if crnn_confidence is not None:
            # Niska pewność CRNN (niepewność) → Wyższa waga [1.0, 2.0]
...
class ContrastiveCapsuleLoss(nn.Module):
    """
    NOWA OPTYMALIZACJA: Contrastive loss dla lepszej separacji kapsułek.
...
class CapsNetLoss(nn.Module):
    ...
    def __init__(self, num_classes, device, reconstruction_weight=0.01, crnn_weights=None, smoothing=0.1, gamma=2.0):
    ...
    def forward(self, images, labels, probs, reconstructions, g_stats, boundaries,
        ...
        # Denormalizacja TARGETU z powrotem do wizualnej przestrzeni [0, 1]
        t_norm = (target * std_val) + mean_val
        t_norm = t_norm.clamp(0, 1)

        # Rekonstrukcja
        r_norm = reconstructions.clamp(0, 1)

        # Liczymy błędy na wspólnej skali
        bce_l = func.binary_cross_entropy(r_norm, t_norm)
        ssim_l = self.ssim(r_norm, t_norm)

        # Balans rekonstrukcji: 70% ostrość pikseli (BCE), 30% struktura globalna (SSIM)
...
def train_and_save(model, train_loader, val_loader, num_classes, device, epochs_to_run, save_path, zone_map, crnn_weights, num_iterations, lr, phase_name="main", checkpoint_dict=None):
    """ Realizuje proces uczenia i adaptacji modelu CapsNet w potoku kaskadowym:
        - Main: Inicjalny trening hybrydowy łączący syntetyczną bazę eMNIST z realnymi
          wycinkami (PURE). Buduje filtry konwolucyjne od razu odporne na teksturę papieru,
          zmienność grubości linii oraz specyficzny szum segmentacji systemu HTR.
        - Fine-Tune: Etap ścisłej adaptacji domenowej, w którym priorytet zyskują rzeczywiste dane docelowe
          (dominacja wycinków PURE na poziomie około 70% w każdym batchu). Faza ta stabilizuje wektorowe
          reprezentacje kapsuł, płynnie transferując ogólną wiedzę o topologii znaków zdobytą w fazie Main na
          specyficzny styl pisma, unikalne zniekształcenia występującące w docelowym korpusie liter wydobytych ze słów.
        - Hard Mining: Precyzyjna faza korekty błędów CRNN. Wykorzystuje mechanizm Hard-Negative
          Mining oraz Triplet Penalty, aby matematycznie rozdzielić reprezentacje znaków, które model
          najczęściej myli wizualnie.
        Zastosowane mechanizmy stabilizacji i zapobiegania przeuczeniu:
        1. Hybrydowa Rekonstrukcja (MSE + SSIM): Wykorzystanie wskaźnika podobieństwa strukturalnego (SSIM)
            wymusza na modelu naukę morfologii znaku (np. domknięcie pętli, kąty łuków). Model musi "rozumieć"
            budowę litery, aby ją zrekonstruować, co uniemożliwia proste zapamiętywanie etykiet.
        2. Dzięki zastosowaniu wagowania próbek model stale widzi bazę eMNIST jako "kotwicę wiedzy".
            Chroni to przed zapominaniem ogólnych kształtów podczas agresywnej adaptacji do
            rozpoznawania części wyrazów, a nie znaków pojedynczych znaków.
        3. Zastosowanie nieliniowych deformacji, zmian perspektywy i szumu sprawia, że sieć każdorazowo optymalizuje
            niezmienniki morfologiczne znaku, a nie konkretne układy pikseli obecne w zbiorze treningowym.
        4. Funkcja straty aktywnie odpycha od siebie pomyłki o wysokim podobieństwie (np. 'g' vs 'q' lub '5' vs 'S'),
            zwiększając margines decyzyjny bez ryzyka destabilizacji wag całego modelu.
        5. System zapisu rekordów oparty na funkcji straty (z tie-breakiem CER) pozwala na uchwycenie momentów,
            w których model staje się bardziej "pewny" swoich decyzji i lepiej separuje cechy, nawet jeśli twardy wynik
            błędu znakowego nie uległ jeszcze zmianie. """
    ...
    best_val_cer = 1.0
    best_loss = float('inf')
    checkpoint = checkpoint_dict if checkpoint_dict is not None else {}

    encoder = CharLabelEncoder()

    # Inicjalizacja strat
    criterion_char = CapsNetLoss(
    ...
    for epoch in range(start_epoch, epochs_to_run):
        model.train()
        running_loss = 0.0
        loop = tqdm(train_loader, desc=f"[{phase_name.upper()}] Epoka {epoch + 1}", total=steps_per_epoch, ncols=120, leave=False, disable=True)
        ...
        for i, batch in enumerate(loop):
            ...
            # Forward pass
            probs, recs, z_logits, g_stats = model(
                inputs, labels, word_context=context_vecs, confidence=confs,
                crnn_probs=crnn_p, boundaries=boundaries
            )

            # Obliczanie strat (Margin/MSE + CrossEntropy dla stref)
            char_loss = criterion_char(inputs, labels, probs, recs, g_stats, boundaries)
            zone_loss = criterion_zone(z_logits, zones.long())

            # Sumaryczna strata z wagą stref (Multi-task learning)
            loss = (char_loss + 0.1 * zone_loss) / ACCUMULATION_STEPS
            loss.backward()
        ...
        def get_cer_score(loader):
            ...
            with torch.no_grad():
                for j, b_v in enumerate(loader):
                    ...
                    v_in, v_lbl, _, v_bnd, v_ctx, v_cnf, *v_rst = b_v_c
                    v_probs, *_ = model(v_in, word_context=v_ctx, confidence=v_cnf,
                                        crnn_probs=(v_rst[0] if v_rst else None), boundaries=v_bnd, num_iterations=num_iterations)
                    t_cer += calculate_cer(torch.max(v_probs, 1)[1], v_lbl, encoder)
                    steps += 1
            return t_cer / steps if steps > 0 else 1.0

        model.eval()
        metrics = {}

        if train_and_save.loader_pure_val is None and os.path.exists(CRNN_CUSTOM_SAMPLES):
            ds_p = HardCharsDataset(CRNN_CUSTOM_SAMPLES, encoder=encoder, case_type="pure", transform=VAL_TRANSFORMS)
            if len(ds_p) > 0:
                train_len = int(0.8 * len(ds_p))
                val_len = len(ds_p) - train_len
                _, val_p = random_split(ds_p, [train_len, val_len])
                train_and_save.loader_pure_val = DataLoader(val_p, batch_size=BATCH_SIZE, num_workers=0)

        if train_and_save.loader_pure_val:
            metrics['Pure'] = get_cer_score(train_and_save.loader_pure_val)

        # Ewaluacja EMNIST
        if val_loader:
            metrics['eMNIST'] = get_cer_score(val_loader)
        ...
        if phase_name == "HARD_MINING":
            ...
            if train_and_save.loader_hard_val:
                metrics['Hard'] = get_cer_score(train_and_save.loader_hard_val)

        # Zapewniamy, że val_cer jest zawsze floatem
        raw_val = metrics.get('Pure')
        if phase_name == "MAIN":
            val_cer = float(raw_val if raw_val is not None else 1.0)
        elif phase_name == "FINE_TUNE":
            val_cer = float(raw_val if raw_val is not None else metrics.get('eMNIST', 1.0))
        elif phase_name == "HARD_MINING":
            # Kaskadowe szukanie metryki z jawnym rzutowaniem
            hard_val = metrics.get('Hard')
            val_cer = float(
                hard_val if hard_val is not None else (raw_val if raw_val is not None else metrics.get('eMNIST', 1.0)))
        else:
            val_cer = float(raw_val if raw_val is not None else 1.0)

        # Logika is_best z zabezpieczeniem przed None
        is_best = False

        # Zakładamy, że best_loss i best_val_cer są zainicjalizowane jako float('inf') na początku skryptu
        current_train_loss = float(train_loss) if train_loss is not None else float('inf')

        if phase_name == "HARD_MINING":
            # W fazie Hard Mining priorytetem może być strata, ale sprawdzamy też CER
            if current_train_loss < best_loss:
                is_best = True
            elif val_cer < best_val_cer:
                is_best = True
        else:
            # Standardowa ścieżka: tylko strata (lub główna metryka)
            if current_train_loss < best_loss:
                is_best = True

        if is_best:
            best_loss = min(train_loss, best_loss)
            best_val_cer = min(val_cer, best_val_cer)
            ...
            tqdm.write(f"[{now()}] NOWY REKORD ({phase_name})! CER: {best_val_cer * 100:.2f}% | Loss: {best_loss:.4f}")
            patience_counter = 0
        else:
            patience_counter += 1
            tqdm.write(f"[{phase_name.upper()}] Brak poprawy ({patience_counter}/{max_patience}).")
...
def train_capsnet_fusion(model, loader, optimizer, criterion, device):
    model.train()
    total_loss = 0

    for images, contexts, labels in tqdm(loader, desc="Uczenie CapsNet"):
        images, contexts, labels = images.to(device), contexts.to(device), labels.to(device)

        optimizer.zero_grad()

        # Przekazanie obrazu z kontekstem, dla lepszych wyników
        output, reconstructions = model(images, word_context=contexts)

        # Obliczanie straty (Margin Loss + Reconstruction Loss)
        loss = criterion(output, labels, images, reconstructions)
...
def run_capsnet_adaptation(model_caps, user_data_path, device):
    """ Dostrajanie CapsNet do charakteru pisma użytkownika.
    Wykorzystuje obrazy rektyfikowane po STN oraz wektory kontekstu. """
    # Przygotowanie danych użytkownika
    user_ds = CapsuleFusionDataset(
        root_dir=user_data_path,
        transform=VAL_TRANSFORMS
    )
    user_loader = DataLoader(user_ds, batch_size=8, shuffle=True)

    # Konfiguracja "Sędziego"
    optimizer = torch.optim.AdamW(model_caps.parameters(), lr=LR_USER_CAPS)
    criterion = CapsNetLoss(num_classes=89, device=device)

    model_caps.train()

    # Zamrażamy backbone, dociągamy tylko routing i głowice decyzyjne
    model_caps.freeze_backbone()

    tqdm.write(f"[{now()}] Start personalizacji CapsNet na {len(user_ds)} wycinkach.")

    for epoch in range(ADAPT_EPOCHS):
        epoch_loss = 0
        for images, contexts, labels in user_loader:
            images, contexts, labels = images.to(device), contexts.to(device), labels.to(device)

            optimizer.zero_grad()

            # Forward pass z realnymi danymi
            probs, reconstructions, z_logits, g_stats = model_caps(
                images,
                word_context=contexts,
                confidence=torch.ones(images.size(0)).to(device),  # Ufamy korektom wprowadzonym przez użytkownika
                crnn_probs=None,  # Na tym etapie CapsNet uczy się czystej formy
                num_iterations=3
            )

            # Obliczamy stratę (Margin + SSIM + MSE)
            loss = criterion(images, labels, probs, reconstructions, g_stats, torch.zeros(images.size(0), 7))

            loss.backward()
            optimizer.step()
            epoch_loss += loss.item()

        tqdm.write(f"      └─> Epoka {epoch + 1}/{ADAPT_EPOCHS} | Loss: {epoch_loss / len(user_loader):.5f}")

    model_caps.unfreeze_backbone()
...
def data_generator(file_paths):
    for path in file_paths:
        # Ładujemy tylko jeden plik/obraz na raz
        data = load_single_item(path)
        processed_data = preprocess(data)
        yield processed_data


class EMNISTWithContext(Dataset):
    def __init__(self, raw_dataset, encoder):
        self.raw_dataset = raw_dataset
        self.encoder = encoder

        # Standardowy maping eMNIST 'byclass'
    ...
    def __getitem__(self, index):
        ...
        return image, torch.tensor(target_label, dtype=torch.long), 1, boundaries, context_vec, torch.tensor(0.0)
...
class RAMCachedEMNIST(Dataset):
    def __init__(self, base_dataset, zone_map, desc="Ładowanie eMNIST do RAM"):
        ...
        for batch in tqdm(temp_loader, desc=desc, disable=True, ncols=100, leave=False):
            imgs, lbls, zones, bounds, ctxs, confs = batch

            # Przechowujemy w float16, aby zmieścić 800k obrazów w ~6GB RAM
            self.images.append(imgs)
            self.labels.append(lbls)
            self.contexts.append(ctxs)

            # Mapowanie stref (wektorowo dla całego batcha)
...
def build_hard_mining_loader(ds_emnist, ds_hard, ds_pure, ds_phsf, batch_size):
    """
    Loader dla Fazy hard mining.
    Dynamicznie łączy zbiory, nadając priorytet błędom CRNN oraz trudnym danym PHSF.
    """
    datasets_to_combine = []
    weights = []
...
def load_single_item(path):
    """ Ładuje pojedynczy obraz do pamięci. """
    return Image.open(path).convert('L')


def preprocess(img_pil):
    """ Podstawowy preprocessing dla pojedynczego obrazu. """
    return crnn_crop_transform(img_pil)


def get_attention_heatmap(self, input_tensor, word_context=None):
    """ Generuje mapę cieplną pokazującą, na czym skupił się CapsNet. """
    self.eval()

    # Przygotowanie tensora do obliczania gradientów
    img_t = input_tensor.clone().detach()
    if img_t.dim() == 3:
    ...
    # Forward pass
    probs, _, _, _ = self.forward(img_t, word_context=word_context)

    # Wybór zwycięskiej klasy i Backpropagation
    conf, _ = torch.max(probs, dim=1)

    self.zero_grad()
    conf.backward()  # Liczymy gradient wyjścia względem wejścia

    # Wyciągamy gradienty. To pokazuje, które piksele miały największy wpływ na pewność decyzji
    gradients = img_t.grad.data.abs().squeeze().cpu().numpy()

    # Normalizacja do zakresu 0-1
    gradients = (gradients - gradients.min()) / (gradients.max() - gradients.min() + 1e-8)
    # Rozmycie Gaussa usuwa szum i zostawia główne skupiska uwagi
    gradients = cv.GaussianBlur(gradients, (5, 5), 0)
    # Ponowna normalizacja po rozmyciu dla lepszego kontrastu
    gradients = (gradients - gradients.min()) / (gradients.max() - gradients.min() + 1e-8)

    # Konwersja na mapę kolorów
    heatmap = cv.applyColorMap((gradients * 255).astype(np.uint8), cv.COLORMAP_JET)

    # Przygotowanie tła
    orig_img = img_t.detach().squeeze().cpu().numpy()
    orig_img = (orig_img * 0.3081) + 0.1307
    orig_img = np.clip(orig_img * 255, 0, 255).astype(np.uint8)
    orig_img_color = cv.cvtColor(orig_img, cv.COLOR_GRAY2BGR)

    # 60% oryginału, 40% mapy ciepła
    result = cv.addWeighted(orig_img_color, 0.6, heatmap, 0.4, 0)

    return result


class HTRPipeline:
    def __init__(self, model_crnn, encoder):
        self.model = model_crnn
        self.encoder = encoder
        self.segmentor = Preprocessing()
        self.line_segmentor = Preprocessing()

    def predict_page_batched(self, word_crops):
        """ Symuluje działanie aplikacji batched-prediction dla listy wycinków słów. """
        self.model.eval()
        results = []
        for word_img, _ in word_crops:
            # Przygotowanie obrazu (konwersja na tensor)
            img_t = VAL_TRANSFORMS(image=word_img)['image'].unsqueeze(0).to(DEVICE)

            with torch.no_grad():
                log_probs, context, _ = self.model(img_t, return_stn=True)

            # Dekodowanie wyniku
            pred_text = self.encoder.decode_greedy(log_probs)[0]

            # Zwracamy metadane potrzebne do miningu
            results.append({
                'crnn_result': pred_text,
                'context_vector': context.squeeze(0),
                'char_probs': torch.exp(log_probs).squeeze(1)
            })
        return results


def mine_errors_from_cvl_pages(pipeline, cvl_db_path, save_root, max_samples=3000):
    """ Przeszukuje bazę CVL, porównuje wynik CRNN z Ground Truth i zapisuje wycinki.
        Zgodnie ze strukturą: save_root / [znak] / [hard_case|pure] / pliki """
    pages_path = os.path.join(cvl_db_path, "pages")
    xml_path = os.path.join(cvl_db_path, "xml")

    # Tworzenie głównego folderu
    os.makedirs(save_root, exist_ok=True)

    count_hard = 0
    page_files = [f for f in os.listdir(pages_path) if f.endswith('.tif')]
    pbar = tqdm(total=max_samples, desc="Wydobywanie błędów CVL", leave=False)

    for page_file in page_files:
        if count_hard >= max_samples: break

        img_path = os.path.join(pages_path, page_file)
        gt_xml_path = os.path.join(xml_path, page_file.replace('.tif', '.xml'))

        if not os.path.exists(gt_xml_path): continue

        try:
            tree = elTree.parse(gt_xml_path)
            root = tree.getroot()

            # Wyciągamy tekst raz, sprawdzamy czy istnieje i czyścimy
            gt_lines = [
                t.strip() for line in root.iter('line')
                if (t := line.get('text')) is not None
            ]

        except (elTree.ParseError, FileNotFoundError, OSError) as e:
            # Przechwytujemy tylko błędy struktury XML lub dostępności pliku
            print(f"[{now()}] Problem z plikiem Ground Truth {gt_xml_path}: {e}")
            continue

        img = cv.imread(img_path, cv.IMREAD_GRAYSCALE)
        # Segmentacja linii
        lines = pipeline.line_segmentor.extract_lines(img)

        for l_idx, line_img in enumerate(lines):
            if l_idx >= len(gt_lines): break

            gt_words = gt_lines[l_idx].split()
            # Segmentacja słów
            word_crops = pipeline.segmentor.extract_atomic_crops(line_img)
            # Predykcja CRNN dla całego batcha słów w linii
            res_batch = pipeline.predict_page_batched(word_crops)

            for i, (crop_info, meta) in enumerate(zip(word_crops, res_batch)):
                word_img, _ = crop_info
                pred_text = meta['crnn_result']
                target_text = gt_words[i] if i < len(gt_words) else ""

                # Analizujemy tylko słowa, gdzie liczba znaków się zgadza (łatwa ekstrakcja 1:1)
                if target_text and len(pred_text) == len(target_text):
                    # Ekstrakcja pojedynczych znaków ze słowa
                    char_crops = pipeline.segmentor.extract_atomic_crops(word_img)

                    for c_idx, (c_img, c_meta) in enumerate(char_crops):
                        if c_idx >= len(target_text): break

                        char_gt = target_text[c_idx]
                        char_pred = pred_text[c_idx]

                        # Wybór podfolderu na podstawie błędu
                        case_subdir = "hard_case" if char_gt != char_pred else "pure"

                        # Bezpieczna nazwa folderu dla znaków specjalnych
                        char_folder = str(ord(char_gt)) if char_gt in r'<>:"/\|?*' else char_gt
                        target_dir = os.path.join(save_root, char_folder, case_subdir)
                        os.makedirs(target_dir, exist_ok=True)

                        # Nazwa pliku i inwersja (CapsNet woli biały znak na czarnym tle)
                        base_fn = f"cvl_{page_file.split('.')[0]}_l{l_idx}_w{i}_c{c_idx}"
                        img_path_save = os.path.join(target_dir, f"{base_fn}.png")
                        cv.imwrite(img_path_save, cv.bitwise_not(c_img))

                        # Zapis metadanych
                        meta_path_save = os.path.join(target_dir, f"{base_fn}.npy")
                        np.save(meta_path_save, {
                            'context_vector': meta.get('context_vector', np.zeros(1024)),
                            'boundaries': c_meta.get('boundaries', [0, 0, 1, 1]),
                            'crnn_probs': meta.get('char_probs', np.zeros(74))[
                                c_idx] if 'char_probs' in meta else np.zeros(74)
                        })
...
class UniversalContextWrapper(Dataset):
    ...
    def __init__(self, dataset, context_size=1024, num_classes=89):
    ...
    def __getitem__(self, idx):
        ...
        # Jeśli dane mają już pełny format bilingwalny
        if len(data) == 8:
            return data

        # Jeśli dane mają 7 elementów (starszy format z crnn_probs, ale bez lang_id) - Angielski
        elif len(data) == 7:
            img, lbl, zone, bnd, ctx, conf, probs = data
            return img, lbl, zone, bnd, ctx, conf, probs, torch.tensor(1, dtype=torch.long)

        # Uzupełniamy o puste prawdopodobieństwa CRNN (74 klasy) oraz domyślny język
        elif len(data) == 6:
            img, lbl, zone, bnd, ctx, conf = data
            return img, lbl, zone, bnd, ctx, conf, torch.zeros(74), torch.tensor(1, dtype=torch.long)

        return data


def run_phase(phase_id, model, device, encoder, current_zone_map, test_loader, num_classes, pipeline):
    ...
    emnist_transform_aggressive = v2.Compose([
        ...
        v2.RandomApply([v2.ElasticTransform(alpha=25.0, sigma=4.0)], p=0.4),
        v2.RandomApply([v2.GaussianBlur(kernel_size=3, sigma=(0.1, 1.5))], p=0.3),
    ...
    if phase_id == 1:
        ...
        train_p_len = int(0.9 * len(full_ds_p))
        ds_p_train, ds_p_val = random_split(full_ds_p, [train_p_len, len(full_ds_p) - train_p_len], generator=torch.Generator().manual_seed(3407))
        ds_p_train_wrapped = UniversalContextWrapper(ds_p_train)

        # Łączymy trzy zbiory
        combined = ConcatDataset([ds_e, ds_p_train_wrapped, ds_phsf])

        # 50% eMNIST, 20% PURE, 30% PHSF — to zapewni, że model nie "zapomni" polskich znaków
        w_e = 0.5 / len(ds_e)
        w_p = 0.2 / len(ds_p_train_wrapped)
        w_ph = 0.3 / len(ds_phsf)
        weights = [w_e] * len(ds_e) + [w_p] * len(ds_p_train_wrapped) + [w_ph] * len(ds_phsf)
    ...
    # Adaptacja do PURE
        ...
        # 40% PURE, 40% PHSF, 20% eMNIST
        w_e = 0.2 / len(ds_e)
        w_p = 0.4 / len(ds_p_train_wrapped)
        w_ph = 0.4 / len(ds_phsf)
        weights = [w_e] * len(ds_e) + [w_p] * len(ds_p_train_wrapped) + [w_ph] * len(ds_phsf)
    ...
    # Korekta błędów i polszczyzna
        ...
        # Kara za błąd na polskim diakrytyku
        ...
        for idx in polish_indices:
            crnn_weights[idx] = 2.5  # Wysoki priorytet dla polszczyzny

        # Import wag z macierzy pomyłek CRNN dla trudnych par
        analysis_path = os.path.join(CRNN_CUSTOM_SAMPLES, "crnn_error_analysis.json")
        if os.path.exists(analysis_path):
            with open(analysis_path, 'r', encoding='utf-8') as f:
                err_data = json.load(f)
            extra_weights = get_crnn_error_weights(err_data['char_true'], err_data['char_pred'], encoder)
            crnn_weights = torch.max(crnn_weights, extra_weights.to(device))

        # Zamrożenie ekstraktora cech – optymalizujemy tylko routing i głowice
        model.freeze_backbone()
...
if __name__ == "__main__":
    ...
    tqdm.write(f"[{now()}] Inicjalizacja globalnego cache eMNIST.")
    base_emnist = datasets.EMNIST(root=DATA_ROOT_EMNIST, split='byclass', train=True, download=True, transform=EMNIST_TRANSFORM)
    cached_emnist_train = RAMCachedEMNIST(EMNISTWithContext(base_emnist, encoder), current_zone_map)
    ...
    ds_test_raw = datasets.EMNIST(root=DATA_ROOT_EMNIST, split='byclass', train=False,
                                  ...
                                  target_transform=hard_test_transform)

    translated_emnist = EMNISTWithContext(ds_test_raw, encoder)
    test_loader = DataLoader(
        dataset=RAMCachedEMNIST(translated_emnist, current_zone_map),
        batch_size=BATCH_SIZE,
        shuffle=False,
        num_workers=0
    )
    ...
    if start_phase <= 3:
        for phase_id in range(start_phase, 4):
            run_phase(phase_id, model, device, encoder, current_zone_map, test_loader, num_classes, pipeline)
    else:
